...
sales <-data0 %>%
  ...
  select(Order_Date_Day, Product_Name, Product_Category,Customer_Name, Customer_Country ) %>%
  rename(date = Order_Date_Day, product_name = Product_Name, product_category = Product_Category,
         customer_name = Customer_Name, customer_country = Customer_Country) %>%
  arrange(date, product_name, product_category, customer_name, customer_country) 
